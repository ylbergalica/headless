...
const CartCard = (props) => {
	...
	return (
		<div className='h-max w-full py-4 px-0 flex items-center justify-around flex-wrap border-t border-zinc-300'>
			<img className={'w-[15%] flex flex-[0_0_100px] lg:flex-[0_0_170px] rounded-md' +' '+ (props.forDrawer ? 'styles.drawerCardImage' : '')} src={props.item.variant.image?.src} alt={props.item.variant.image?.altText} />

			<div className={'w-[50%] lg:min-w-0 pl-2 flex flex-col items-start' +' '+ (props.forDrawer ? 'styles.drawerCardOptions' : '')}>
			...
			<div className={'w-max flex justify-end' +' '+ (props.forDrawer ? 'styles.drawerCardPriceHolder' : '')}>

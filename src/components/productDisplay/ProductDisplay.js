...
const ProductDisplay = ({ item }) => {
	...
	return (
		<div className='lg:max-h-[30rem] w-full m-0 flex flex-col lg:flex-row items-center justify-center'>
			...
			<div className='max-h-[30rem] w-full lg:w-1/2 py-4 px-6 lg:px-12 order-3'>
				...
				<h1 className='text-[1.6rem] lg:text-5xl mb-7'>${item.variant.price}</h1>
				...
				<div>
					<ATCButton variantID={item.variant.id} quantity={quantityValue} />
				</div>
			</div>
		</div>
	)
